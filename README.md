...
- ES Modules (ESM) using `import/export` - the offical standard for packaging JavaScript for reuse, supported in Node.js going forward. This is set using `"type": "module"` in `package.json`. Note that if you are using this as a starting point, you should use `import` over the traditional CommonJS `require()` syntax you may see in a lot of Node.js examples.
